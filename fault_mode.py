...
def run_fault_mode(db_config, pg_data_dir, fault_mode, k_workers, num_transactions_per_worker,
    ...
    if fault_mode == 'single_injection':
        ...
        monitor_process = Process(target=monitor_and_inject_fault,
        ...
        print(f"阶段 1: 故障将在执行 {first_injection_transactions} 个事务后注入。") 

        transaction_counter.value = 0
        monitor_process_1 = Process(target=monitor_and_inject_fault,
                                    args=(transaction_counter, first_injection_transactions, pg_data_dir))
        ...
        print(f"\n阶段 2: 开始新的事务。故障将在执行 {second_injection_transactions} 个事务后注入。") 


        transaction_counter.value = 0
        monitor_process_2 = Process(target=monitor_and_inject_fault,
                                    args=(transaction_counter, second_injection_transactions, pg_data_dir))
        monitor_process_2.start()

        num_transactions_per_worker_phase2 = second_injection_transactions // k_workers
        if second_injection_transactions % k_workers != 0:
            num_transactions_per_worker_phase2 += 1
    ...
    elif fault_mode == 'multi_table_delete_fault_injection':
        print(f"运行在多表删除故障注入模式。") 
        ...
        worker_func_with_args = partial(worker_function,
                                        ...
                                        fault_point='delete_product_after_step2')
    ...
    elif fault_mode == 'multi_table_price_fault_injection':
        print(f"运行在多表价格故障注入模式。") 
        ...
        worker_func_with_args = partial(worker_function,
                                        ...
                                        pg_data_dir=pg_data_dir,
                                        fault_mode_specific_injection=True,
                                        fault_point='modify_price_after_order_item_update')
